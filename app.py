...
# --- IMPORT UTILS ---
# Ensure you have utils/ folder created with these files!
try:
    from utils.vision import analyze_crop_disease, verify_sustainable_practice
    from utils.insurance import check_weather_oracle, trigger_payout_transaction, generate_insurance_policy
    from utils.carbon import calculate_green_score, mint_carbon_tokens
    from utils.history import load_history, save_transaction
    from utils.rag import agent_chain
    ...
    from utils.trustmesh import mesh_agent
    from utils.ondc import broadcast_to_ondc, generate_invoice
    from utils.shared_db import get_latest_certificate
except ImportError:
    # Fallback for display purposes if utils are missing
    st.error("Utils modules missing. Please ensure the 'utils' folder exists.")
...
# Create a basic style.css if it doesn't exist to prevent errors
...
def generate_quality_pdf(result):
    ...
    pdf.cell(200, 10, txt=f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=1)
    filename = "quality_certificate.pdf"
    # In a real app, save to a temp path
    # pdf.output(filename) 
    return filename

# ==========================================
# 1. ROOT LOGIN LAYER (ROLE SELECTION)
# ==========================================
if st.session_state.role is None:
    st.markdown("<div style='text-align: center; padding: 50px;'>", unsafe_allow_html=True)
    st.markdown("<h1 style='font-size: 3.5rem;'>🌾 VeriYield Neural-Chain</h1>", unsafe_allow_html=True)
    st.markdown("<p style='font-size: 1.5rem; color: #666;'>Select your ecosystem role to proceed</p>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    
    c1, c2, c3 = st.columns([1, 2, 1])
    with c2:
        col_farmer, col_buyer = st.columns(2)
        
        with col_farmer:
            st.markdown("""
            <div style="text-align: center; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background: white;">
                <h2 style="font-size: 3rem;">👨‍🌾</h2>
                <h3>Farmer</h3>
                <p>Generate Trust, Insure Crops, Access Credit.</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Login as Farmer", use_container_width=True):
        ...
        with col_buyer:
            ...
            <div style="text-align: center; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background: white;">
                <h2 style="font-size: 3rem;">👔</h2>
                <h3>Buyer</h3>
                <p>Verify Quality, Analyze Pricing, Negotiate.</p>
            </div>
            """, unsafe_allow_html=True)
            if st.button("Login as Buyer", use_container_width=True):
                st.session_state.role = "Buyer"
                st.session_state.logged_in = True # Buyers skip ZK auth for this demo
                st.rerun()

# ==========================================
# 2. FARMER DASHBOARD FLOW
# ==========================================
elif st.session_state.role == "Farmer":
    
    # --- FARMER AUTHENTICATION LAYER (ZK-IDENTITY) ---
    if not st.session_state.logged_in:
        with st.sidebar:
        ...
        st.markdown("<div style='text-align: center; padding: 40px;'>", unsafe_allow_html=True)
        st.markdown("<h1>🔐 Farmer ZK-Login</h1>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
            ...
            with st.container():
                ...
                if st.button("Connect Identity (Zero-Knowledge)", type="primary", use_container_width=True, disabled=not consent):
                    with st.spinner("Generating Cryptographic Proof..."):
                        time.sleep(1.5)
                        st.success("✅ Proof Verified: Resident of Maharashtra")
                        time.sleep(0.5)
                    
                    with st.status("🚜 Accessing AgriStack Registry...", expanded=True) as status:
                        st.write("📡 Connecting to UFSI Gateway...")
                        time.sleep(0.8)
                        st.write("📍 Found Land Parcel: **Survey No. 45/2A (Nashik)**")
                        time.sleep(0.5)
                        status.update(label="✅ Digital Twin Linked!", state="complete", expanded=False)
    ...
    # --- MAIN FARMER DASHBOARD ---
        ...
        with st.sidebar:
            ...
            try:
                ...
                balance = 0
                for h in history:
                    if "ETH" in str(h.get('details', '')): balance += 0.5 
                st.metric("Wallet Balance", f"{balance} ETH")
            except:
                st.metric("Wallet Balance", "0 ETH")
        ...
        st.markdown("## 🌾 VeriYield Farmer Dashboard")
            ...
            with col_input:
                ...
                if st.button("🔄 Sync Field Data", type="primary"):
                    with st.spinner("Handshaking with IoT Sensors (LoRaWAN)..."):
                        ...
                        # Also set last_analysis for Tab 2
                        st.session_state.last_analysis = st.session_state.crop_data
                ...
                if 'sensor_history' in st.session_state:
                    ...
                    with bc1:
                        ...
                        if qrcode:
                            ...
                            st.warning("Install 'qrcode' module to see QR")
...
# ==========================================
# 3. BUYER DASHBOARD FLOW
# ==========================================
elif st.session_state.role == "Buyer":
    from utils.buyer_agent import buyer_brain
    
    # Buyer Sidebar
    with st.sidebar:
        st.title("👔 Buyer Admin")
        if st.button("🚪 Logout"):
            st.session_state.logged_in = False
            st.session_state.role = None
            st.rerun()
            
    st.markdown("## 👔 Enterprise Procurement Console")
    st.caption("Verify quality, analyze fair price, and negotiate.")
    
    # --- STEP 1: SCANNER ---
    st.markdown("### 1️⃣ Verify Origin")
    
    col_scan, col_data = st.columns([1, 2])
    with col_scan:
        st.markdown("""
        <div style="border: 2px dashed #ccc; padding: 40px; text-align: center; border-radius: 10px; background: white;">
            📷 <b>QR Scanner Active</b><br>
            Point camera at Crate Label
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("📱 Simulate Scan (Read Blockchain)"):
            with st.spinner("Decoding QR & Fetching Metadata..."):
                time.sleep(1.5)
                # Fetch the latest cert minted by the farmer from shared DB
                cert = get_latest_certificate()
                
                if cert:
                    st.session_state.scanned_cert = cert
                    st.success("✅ Certificate Verified!")
                else:
                    st.error("No certificates found on chain.")

    # --- STEP 2: ANALYSIS DASHBOARD ---
    if 'scanned_cert' in st.session_state:
        cert = st.session_state.scanned_cert
        harvest = cert['current_harvest']
        reputation = cert['farmer_reputation']
        
        st.divider()
        
        # A. TRUST SCORE ANALYSIS
        st.markdown("### 2️⃣ Quality Intelligence")
        c1, c2 = st.columns([1, 2])
        
        with c1:
            score = harvest['trust_score']
            st.metric("Trust Score", f"{score}/100", delta="Verified by IoT")
            st.markdown(f"**Grade:** {harvest['grade']}")
        
        with c2:
            st.markdown("**🤖 AI Explanation (Why this score?):**")
            with st.spinner("Analyzing sensor logs..."):
                # Call the Buyer Agent to explain
                explanation = buyer_brain.explain_trust_score(cert)
                st.info(explanation)

        # B. FARMER HISTORY (Reputation)
        st.markdown("### 3️⃣ Farmer Reliability History")
        
        # Create a quick dataframe for the sparkline
        history_data = [h['score'] for h in reputation['score_history']]
        history_data.append(score) # Add current
        
        col_hist_chart, col_hist_stat = st.columns([3, 1])
        with col_hist_chart:
            st.line_chart(history_data, height=150)
            st.caption("Performance over last 4 harvests")
        with col_hist_stat:
            st.metric("Avg Score", f"{reputation['average_score']}")
            st.metric("Total Deals", f"{reputation['total_harvests_verified']}")

        # C. PRICING & NEGOTIATION
        st.divider()
        st.markdown("### 4️⃣ Live Market & Pricing")
        
        # Get AI Price Analysis
        with st.spinner("Fetching Mandi Rates..."):
            price_analysis_json = buyer_brain.get_fair_price_analysis(harvest['crop'])
            try:
                price_data = json.loads(price_analysis_json)
            except:
                price_data = {"market_price": "₹22/kg", "suggested_premium": "₹25/kg", "reason": "AI Error"}

        p1, p2, p3 = st.columns(3)
        p1.metric("Live Market Rate", price_data.get('market_price'))
        p2.metric("Suggested Fair Price", price_data.get('suggested_premium'), delta="Includes Quality Premium")
        p3.info(f"💡 **AI Reasoning:** {price_data.get('reason')}")
        
        # Negotiation Chat
        with st.expander("💬 Negotiate with Farmer (Secure Channel)", expanded=True):
            user_offer = st.text_input("Enter your Offer (₹/kg)", placeholder="e.g., 24")
            if st.button("📨 Send Offer"):
                # Extract number from user input (handles both "24" and "i want 24 rupees")
                import re
                numbers = re.findall(r'\d+', str(user_offer))
                if numbers:
                    offer_value = int(numbers[0])
                    st.toast("Offer sent directly to Farmer's WhatsApp!")
                    time.sleep(1)
                    st.chat_message("user").write(f"I offer ₹{offer_value}/kg for this lot.")
                    time.sleep(1.5)
                    st.chat_message("assistant", avatar="👨‍🌾").write(f"Sir, your AI shows my Grade is A+. Market is ₹{price_data.get('market_price')}. Can we do ₹{offer_value+1}?")
                else:
                    st.error("Please enter a valid price (e.g., 24 or '24 rupees')")